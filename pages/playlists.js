...
const PlaylistCard = ({ playlist, isMobile = false }) => {
  ...
  // 🆕 CEK STATUS METADATA - TAPI HANYA TAMPILKAN UNTUK YANG BELUM ENHANCED
  ...
  const isEnhanced = hasMetadata && !isAIFallback; // 🆕 Enhanced = AI yang berhasil
  ...
  return (
    ...
    >
      {/* 🆕 METADATA STATUS INDICATOR - HANYA TAMPIL UNTUK YANG BELUM ENHANCED */}
      {!isEnhanced && ( // 🆕 Hanya tampilkan jika BUKAN enhanced
        <div style={{
          position: 'absolute',
          ...
          {/* 🆕 GENERATE BUTTON (for admin/owner) - HANYA UNTUK YANG BELUM ENHANCED */}
          {isOwner && (
            <button
      ...
      <div style={{
        ...
        position: 'relative',
        zIndex: 1,
        // 🆕 SESUAIKAN MARGIN TOP: jika enhanced, margin normal; jika belum enhanced, ada space untuk indicator
        marginTop: (!isEnhanced && (hasMetadata || isOwner)) ? (isMobile ? '1.5rem' : '1.8rem') : '0'
      }}>
        <h3 style={{
          margin: '0 0 0.5rem 0',
      ...
      {/* 🆕 METADATA PREVIEW - TETAP DITAMPILKAN UNTUK SEMUA */}
